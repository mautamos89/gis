var serverip = 'http://localhost:8080/' // server
var daturl = serverip + 'geoserver/cali/wms' // geoserver workspace

var mapView = new ol.View({
    center: ol.proj.fromLonLat([-76.536, 3.350]),
    zoom: 14.3
});
...
// Useful for No base map legend group
...
var baseGroup = new ol.layer.Group({
    title: 'Mapa base',
    fold: true,
    layers: [NonTile,SatTile, osmTile]
});
...
map.addLayer(osmTile); // 'Open street map'
map.addLayer(SatTile); // 'Imagen satelital'
//map.addLayer(NonTile)
// Add basemaps: end */

//PostGIS Layers
var CalTerTil = new ol.layer.Tile({
    title: "Terrenos catastrales",
    source: new ol.source.TileWMS({
        url: daturl,
        params: { 'LAYERS': 'cali:Terrenos catastrales', 'TILED': true},
        serverType: 'geoserver',
        crossOrigin: 'anonymous',
        visible: true
    })
});
//map.addLayer(CalTerTil);
var CalRioTil = new ol.layer.Tile({
    title: "Ríos",
    source: new ol.source.TileWMS({
        url: daturl,
        params: { 'LAYERS': 'cali:Ríos', 'TILED': true },
        serverType: 'geoserver',
        crossOrigin: 'anonymous',
        visible: true
    })
});
var CalSueExpTil = new ol.layer.Tile({
    title: "Suelo de expansión",
    source: new ol.source.TileWMS({
        url: daturl,
        params: { 'LAYERS': 'cali:Suelo de expansión', 'TILED': true },
        serverType: 'geoserver',
        crossOrigin: 'anonymous',
        visible: true
    })
});
var CalLinFerTil = new ol.layer.Tile({
    title: "Línea férrea",
    source: new ol.source.TileWMS({
        url: daturl,
        params: { 'LAYERS': 'cali:Línea férrea', 'TILED': true },
        serverType: 'geoserver',
        crossOrigin: 'anonymous',
        visible: true
    })
});
var CalNomViaTil = new ol.layer.Tile({
    title: "Vías",
    source: new ol.source.TileWMS({
        url: daturl,
        params: { 'LAYERS': 'cali:Vías', 'TILED': true },
        serverType: 'geoserver',
        crossOrigin: 'anonymous',
        visible: true
    })
});
var CalBarTil = new ol.layer.Tile({
    title: "Barrios",
    source: new ol.source.TileWMS({
        url: daturl,
        params: { 'LAYERS': 'cali:Barrios', 'TILED': true },
        serverType: 'geoserver',
        crossOrigin: 'anonymous',
        visible: true
    })
...
var CalComTil = new ol.layer.Tile({
    title: "Comunas",
    source: new ol.source.TileWMS({
        url: daturl,
        params: { 'LAYERS': 'cali:Comunas', 'TILED': true },
        serverType: 'geoserver',
        crossOrigin: 'anonymous',
        visible: true
    })
});
var CalHumTil = new ol.layer.Tile({
    title: "Humedales",
    source: new ol.source.TileWMS({
        url: daturl,
        params: { 'LAYERS': 'cali:Humedales', 'TILED': true },
        serverType: 'geoserver',
        crossOrigin: 'anonymous',
        visible: true
    })
...
// Add layers
/* map.addLayer(CalSueExpTil); // 'Suelo de expansión'
map.addLayer(IndiaDsTile); // 'Barrios'
map.addLayer(CalComTil); // 'Comunas'
map.addLayer(CalNomViaTil); // 'Vías'
map.addLayer(CalLinFerTil); // 'Línea férrea'
map.addLayer(CalRioTil); // 'Ríos'
map.addLayer(IndiaStTile); // 'Humedales'
map.addLayer(CalTerTil); // 'Terrenos catastrales'
 */
...
var overlayGroup1 = new ol.layer.Group({
    ...
    fold: true,
    //layers: [IndiaStTile,CalRioTil,CalLinFerTil,CalNomViaTil,CalComTil,IndiaDsTile,CalSueExpTil]
    layers: [CalSueExpTil,CalBarTil,CalComTil,CalNomViaTil,CalLinFerTil,CalRioTil,CalHumTil]
    //layers: [IndiaDsTile,CalComTil]
});
...
// Coordinates on map: end
// Scale control on map: start
...
// Scale control on map: end
// Pop-up info on map: start
...
map.on('singleclick', function (evt) {
    if (featureInfoFlag) {
        ...
        var url = CalTerTil.getSource().getFeatureInfoUrl(evt.coordinate, resolution, 'EPSG:3857', {
            'INFO_FORMAT': 'application/json',
            //'propertyName': 'id_predio,npn,idmanzana',
            'propertyName': 'id_predio,npn,numepred,idmanzana',
        });
        ...
        if (url) {
            $.getJSON(url, function (data) {
                ...
                content.innerHTML =
                "<p> <strong> ID predio : </strong> <p>" + props.id_predio +
                "</p> <br> <strong> NPN : </strong> <p> " + props.npn +
                "</p> <br> <strong> Número predial : </strong> <p>" + props.numepred +
                "</p> <br> <strong> ID manzana : </strong> <p>" + props.idmanzana + "</p>";
                /*"<h3> ID predio : </h3> <p>" + props.id_predio.toUpperCase() +
                "</p> <br> <h3> NPN : </h3> <p>" + props.npn.toUpperCase() +
                "</p> <br> <h3> ID manzana : </h3> <p>" + props.idmanzana.toUpperCase() + "</p>";*/
...
var formatLength = function (line) {
    ...
    if (length > 1000) {
        ...
        output = Math.round(length * 100) / 100 + ' ' + 'mt';
    }
    return output;
};
...
var formatArea = function (polygon) {
    ...
    if (area > 1000000) {
        ...
        output = Math.round(area * 100) / 100 + ' ' + 'mt<sup>2</sup>';
    }
    return output;
};
...
function addMapLayerList() {
    $(document).ready(function () {
        $.ajax({
            type: "GET",
            url: "http://localhost:8080/geoserver/cali/wfs?request=getCapabilities",
            dataType: "xml",
...
$(function () {
    ...
    document.getElementById("selectAttribute").onchange = function () {
        ...
        operator.options[0] = new Option('Seleccione', "");

        if (value_type == 'xsd:short' || value_type == 'xsd:int' || value_type == 'xsd:double') {
            var operator1 = document.getElementById("selectOperator");
            operator1.options[1] = new Option('Mayor [>]', '>');
            operator1.options[2] = new Option('Menor [<]', '<');
            operator1.options[3] = new Option('Igual [=]', '=');
        }
        else if (value_type == 'xsd:string') {
            var operator1 = document.getElementById("selectOperator");
            operator1.options[1] = new Option('Incluye [Like]', 'Like');
            operator1.options[2] = new Option('Igual [=]', '=');
        }
    }
    ...
    document.getElementById('attQryRun').onclick = function () {
        ...
        if (layer.options.selectedIndex == 0) {
            alert("Seleccione Capa");
        } else if (attribute.options.selectedIndex == -1) {
            alert("Seleccione Atributo");
            ...
            //var url = "http://localhost:8080/geoserver/cali/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + value_layer + "&CQL_FILTER=" + "strToLowerCase(" + value_attribute + ")+" + value_operator + "+'" + value_txt + "'&outputFormat=application/json"
            var url = "http://localhost:8080/geoserver/cali/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + value_layer + "&CQL_FILTER=" + value_attribute + "+" + value_operator + "+'" + value_txt + "'&outputFormat=application/json"
            // console.log(url);
