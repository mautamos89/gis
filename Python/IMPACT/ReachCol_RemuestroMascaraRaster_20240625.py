...
# Layer de máscara
layer_mascara = arcpy.MakeFeatureLayer_management(capa_mascara,'layer_mascara_lyr',"lower(NOM_MUNICI) LIKE 'san miguel%' and lower(NOM_DEPART) like 'putu%'")
# Carpeta de entrada
...
# Carpeta de salida
ruta_carpeta_out = r'D:\ACTED\IMPACT COL - General\5_GIS\2_Projects\2024_XXX_ECHO\ABA\COL2301_PUTUMAYO\2.WORKSPACE\Indicadores\SAN_MIGUEL'
# Filtro por prefijo
filtro_nombre = 'vegseason'
...
for i in rasterlist:
    ...
    out_filename = os.path.join(ruta_carpeta_out, f"{i[:-4].lower()}_{tamano_celda}m_mask.tif")
    arcpy.management.ProjectRaster(i,output_raster,sis_prj,'CUBIC',tamano_celda)
    outExtractByMask = arcpy.sa.ExtractByMask(output_raster, layer_mascara, 'INSIDE')
    outExtractByMask.save(out_filename)
    print(f'Raster procesado: {i}')
